...
def parse_args():
	...
	qc_arg.add_argument('--testcase', choices=['lind', 'cs_au', 'ns_cs', 'ns_lind', 'Heis_lab',
						'dipole', 'XmonTLS', 'Xmon_nb','NoBath','dipole_VarStr', 'dipole_VarStr_lab', 
...
def train(seed, exp_dir):
	...
	if args.approach == 'qaoa':
		...
		if args.T_tot!= config[args.exp_name]['T_tot']:
		...
		quma_bang = sys_setup.setup(args, couplings=config[args.exp_name]['couplings'])
		print("The MLI from Hamiltonian Switching is", -np.log10(1 - quma_bang.get_reward(config[args.exp_name]['protocol'])))
